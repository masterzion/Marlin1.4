...
void AnycubicTFTClass::HandleSpecialMenu()
{
   if(strcmp(SelectedDirectory, "<special menu>")==0) {
    SpecialMenu=true;
  } else if (strcmp(SelectedDirectory, "<heatbed on>")==0) {
    SERIAL_PROTOCOLLNPGM("Special Menu: Heatbed ON");
    ...
    SpecialMenu=false;
     } else if (strcmp(SelectedDirectory, "<go to y220>")==0) {
    SERIAL_PROTOCOLLNPGM("Special Menu: Go to Y220");
    enqueue_and_echo_commands_P(PSTR("G28 Y\nG1 Y200"));  
   } else if (strcmp(SelectedDirectory, "<heatbed off>")==0) {
    SERIAL_PROTOCOLLNPGM("Special Menu: Heatbed OFF");
    enqueue_and_echo_commands_P(PSTR("M140 S0"));
   } else if (strcmp(SelectedDirectory, "<parkposition>")==0) {
    SERIAL_PROTOCOLLNPGM("Special Menu: Parkposition");
    enqueue_and_echo_commands_P(PSTR("G27"));
    } else if (strcmp(SelectedDirectory, "<home>")==0) {
    SERIAL_PROTOCOLLNPGM("Special Menu: Home");
    enqueue_and_echo_commands_P(PSTR("G28"));
  }
}
...
void AnycubicTFTClass::Ls()
{
  if (SpecialMenu) {
    switch (filenumber) {
      case 0: // First Page
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<Heatbed ON>");
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<Heatbed ON>");
      ...
      case 4: // Second Page
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<Parkposition>");
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<Parkposition>");
      ...
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<FilamentChange Resume>");
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<Home>");
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<Home>");
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<GO to Y220>");
      ANYCUBIC_SERIAL_PROTOCOLLNPGM("<GO to Y220>");
      break;
    }
  }
